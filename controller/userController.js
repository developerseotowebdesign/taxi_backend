...
const sendLogOTP = async (phone, otp) => {
  try {
    ...
    const queryParams = querystring.stringify({
      ...
      from: "GREENV",
      to: phone,
      text: `Your login OTP is ${otp} for valetwale.in Green Valet Parking Solutions`,
    });
...
const RecCar = async (phone) => {
  try {
    ...
    const queryParams = querystring.stringify({
      ...
      from: "GREENV",
      to: phone,
      text: `We have received your car thank you for using valetwale.in Green Valet Parking Solutions`,
    });
    const url = `https://pgapi.smartping.ai/fe/api/v1/send?${queryParams}`;
...
const CarDel = async (phone, otp) => {
  try {
    ...
    const queryParams = querystring.stringify({
      ...
      from: "GREENV",
      to: phone,
      text: `Your car delivery otp is ${otp} please share it with driver if you have received the car valetwale.in Green Valet Parking Solutions`,
    });
    const url = `https://pgapi.smartping.ai/fe/api/v1/send?${queryParams}`;
...
export const SignupNewUser = async (req, res) => {
  try {
    ...
    // Create a new user
    const user = new userModel({ phone, status: "1" });
